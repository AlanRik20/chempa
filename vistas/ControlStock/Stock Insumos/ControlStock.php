...
.otro-titulo h2 {
    ...
    <div class="contPag">
        ...
        <table class="tabla1">
           ...
           LEFT JOIN proveedores pr ON i.id_proveedor = pr.id_proveedor
           LEFT JOIN personas p ON pr.id_persona = p.id_persona
           LEFT JOIN stock_insumos si ON i.id_insumo = si.id_insumo
           LEFT JOIN tipo_medida tm ON i.id_tipo_medida = tm.id_tipo_medida
           WHERE i.id_proveedor>1 OR i.id_proveedor IS NULL
           ...
           ");
            while($resultado = mysqli_fetch_assoc($mostrar)){ 
                ...
                if ($resultado['id_proveedor'] === null) {
                    echo"<td>Sin proveedor relacionado</td>";
                }
                else{
